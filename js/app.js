...
async function initializeApp(user) {
    ...
    // Actualizar menú móvil con datos del usuario
    ...
    if (mobileUserAvatar) {
        if (user.photoURL) {
            mobileUserAvatar.style.backgroundImage = `url(${user.photoURL})`;
            mobileUserAvatar.style.backgroundSize = 'cover';
            mobileUserAvatar.style.backgroundPosition = 'center';
            mobileUserAvatar.textContent = '';
        } else {
            mobileUserAvatar.textContent = user.avatar || firstName.charAt(0);
        }
    }
